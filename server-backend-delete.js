// Backend variant with soft-delete support (for testing on alternate port)
...
app.get('/api/comments/:datasetId',(req,res)=>{
  ...
  const s = readJson(); res.json({ok:true,comments:(s[datasetId]||[]).filter(c=>!c.deleted)})
})
...
app.post('/api/comments/:datasetId', requireApiKey, (req,res)=>{
  ...
  if(db){ db.run('INSERT INTO comments (datasetId,user,text,time,avatar,likes,liked,deleted) VALUES (?,?,?,?,?,?,?,?)',[datasetId,user,text,now,(user||'A').slice(0,2).toUpperCase(),0,0,0], function(err){ if(err) return res.status(500).json({ok:false}); db.get('SELECT * FROM comments WHERE id=?',[this.lastID],(e,row)=>{ if(e) return res.status(500).json({ok:false}); res.json({ok:true,comment:{ id:row.id,user:row.user,text:row.text,time:row.time,avatar:row.avatar,likes:row.likes,liked:!!row.liked,replies:[] }}) }) }); return }
  const s = readJson(); const c = { id: Date.now(), user, text, time: now, avatar: (user||'A').slice(0,2).toUpperCase(), likes:0, liked:false, replies:[], deleted:false }; s[datasetId] = [c,...(s[datasetId]||[])]; writeJson(s); res.json({ok:true,comment:c})
})
...
app.post('/api/comments/:datasetId/:commentId/reply', requireApiKey, (req,res)=>{
  ...
  const now = new Date().toISOString()
  if(db){
    db.get('SELECT id FROM comments WHERE id=? AND datasetId=? AND (deleted IS NULL OR deleted=0)',[Number(commentId),datasetId],(err,row)=>{
      if(err||!row) return res.status(404).json({ok:false})
      db.run('INSERT INTO replies (commentId,user,text,time,avatar) VALUES (?,?,?,?,?)',[Number(commentId),user,text,now,(user||'A').slice(0,2).toUpperCase()], function(err){ if(err) return res.status(500).json({ok:false}); db.get('SELECT * FROM replies WHERE id=?',[this.lastID],(e,row)=>{ if(e) return res.status(500).json({ok:false}); res.json({ok:true,reply:row}) }) })
    })
    return
  }
  const s = readJson(); const reply = { id: Date.now(), user, text, time: now, avatar: (user||'A').slice(0,2).toUpperCase() }; let found=false; s[datasetId] = (s[datasetId]||[]).map(c => { if(c.id===Number(commentId) && !c.deleted){ found=true; return {...c, replies:[...(c.replies||[]), reply]} } return c }); if(!found) return res.status(404).json({ok:false}); writeJson(s); res.json({ok:true,reply})
})

app.post('/api/comments/:datasetId/:commentId/toggle-like', requireApiKey, (req,res)=>{
  const {datasetId,commentId}=req.params
  if(db){ db.get('SELECT * FROM comments WHERE id=? AND datasetId=? AND (deleted IS NULL OR deleted=0)',[Number(commentId),datasetId],(err,row)=>{ if(err||!row) return res.status(404).json({ok:false}); const liked = row.liked?0:1; const likes = liked? (row.likes||0)+1 : Math.max(0,(row.likes||0)-1); db.run('UPDATE comments SET liked=?,likes=? WHERE id=?',[liked,likes,Number(commentId)],(e)=>{ if(e) return res.status(500).json({ok:false}); db.get('SELECT * FROM comments WHERE id=?',[Number(commentId)],(er,updated)=>{ res.json({ok:true,comment:{...updated,liked:!!updated.liked}}) }) }) }); return }
  const s = readJson(); let found=false; s[datasetId] = (s[datasetId]||[]).map(c=>{ if(String(c.id)===String(commentId) && !c.deleted){ found=true; const liked = !c.liked; const likes = liked? (c.likes||0)+1 : Math.max(0,(c.likes||0)-1); return {...c,liked,likes} } return c }); if(!found) return res.status(404).json({ok:false}); writeJson(s); res.json({ok:true,comment:(s[datasetId]||[]).find(c=>String(c.id)===String(commentId))})
})

app.delete('/api/comments/:datasetId/:commentId', requireApiKey, (req,res)=>{
  const {datasetId,commentId}=req.params
  if(db){
    db.run('UPDATE comments SET deleted=1 WHERE id=? AND datasetId=?',[Number(commentId),datasetId], function(err){ if(err) return res.status(500).json({ok:false}); if(this.changes===0) return res.status(404).json({ok:false}); res.json({ok:true}) })
    return
  }
  const s = readJson(); let found=false; s[datasetId] = (s[datasetId]||[]).map(c=>{ if(String(c.id)===String(commentId)){ found=true; return {...c, deleted:true} } return c }); if(!found) return res.status(404).json({ok:false}); writeJson(s); res.json({ok:true})
})
